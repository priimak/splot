...
class Figure(
  ...
  /**
   * Convenience function that can be used instead of fig += SimpleZMapPlot(...) for plotting x,y->z function on
   ...
   * @param inDomain domain function which can be used arbitray domains in x-y plane where plotting on
   *                 the function f will happen; by default it is assigned to special function "derrivedDomain" which
   *                 means that it will be plotted on xDomain, yDomain rectangle
   */
